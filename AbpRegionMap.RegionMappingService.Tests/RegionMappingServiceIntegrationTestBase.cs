...
public abstract class RegionMappingServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(AbpRegionMapRegionMappingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(AbpRegionMapRegionMappingServiceModule).Assembly.FullName} assembly!");
        }
